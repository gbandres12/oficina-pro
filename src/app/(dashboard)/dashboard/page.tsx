...
export default function DashboardPage() {
    ...
    const statusLabel: Record<(typeof activeOrders)[number]['status'], string> = {
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
                <div className="xl:col-span-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Card className="bg-amber-500 text-white border-none shadow-lg">
                            <CardContent className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                ...
                                <Button variant="ghost" className="text-white hover:bg-white/10 font-bold text-xs px-0 sm:px-3" onClick={() => router.push('/gestao-patio')}>
                        ...
                        <Card className="bg-primary text-white border-none shadow-lg">
                            <CardContent className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                ...
                                <Button variant="ghost" className="text-white hover:bg-white/10 font-bold text-xs px-0 sm:px-3" onClick={() => router.push('/financeiro')}>
                    ...
                    <Card className="border-none shadow-xl">
                        <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-50 dark:border-slate-800 pb-6">
                            <div>
                            ...
                            <Button variant="outline" size="sm" className="font-bold border-primary text-primary rounded-xl w-full sm:w-auto" onClick={() => router.push('/gestao-patio')}>
                        ...
                        <CardContent className="pt-6">
                            <div className="space-y-3">
                                {activeOrders.map((order) => (
                                    <div key={order.plate} className="rounded-2xl border bg-slate-50/30 dark:bg-slate-900/50 p-4 md:p-5 hover:bg-white dark:hover:bg-slate-900 transition-all border-transparent hover:border-slate-200 hover:shadow-md">
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                            <div className="flex items-center gap-4">
                                                <div className="flex flex-col items-center justify-center w-16 h-10 border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-lg">
                                                ...
                                                <div>
                                                    <div className="font-black text-sm md:text-base text-slate-800 dark:text-white uppercase tracking-tight">{order.model}</div>
                                                    <div className="text-[10px] text-muted-foreground font-bold">{order.customer}</div>
                                                </div>
                                            </div>
                                            <div className="flex flex-wrap items-center gap-3 md:justify-end">
                                                <Badge variant="outline" className={`text-[10px] font-black border-none py-1 px-2 ${order.parts === 'OK' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                                                    PEÇAS: {order.parts}
                                                </Badge>
                                                <div className="w-full md:w-32 h-1.5 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                                                    <div className="h-full bg-primary" style={{ width: `${order.progress}%` }} />
                                                </div>
                                                <Badge className={`${order.status === 'IN_PROGRESS' ? 'bg-blue-600' : order.status === 'WAITING_PARTS' ? 'bg-amber-500' : 'bg-emerald-600'} rounded-lg text-[10px] font-black py-1 px-3`}>
                ...
                <div className="xl:col-span-4 space-y-6">
                    ...
                    <Card className="border-none shadow-xl bg-slate-900 text-white overflow-hidden relative group">
                        ...
                        <CardContent>
                            <div className="border-2 border-dashed border-slate-700 rounded-2xl p-8 flex flex-col items-center justify-center text-center space-y-4 bg-white/5 backdrop-blur-sm">
                                <Upload className="w-12 h-12 text-primary animate-bounce" />
                                <div className="text-sm font-bold text-slate-300">Solte o arquivo XML aqui</div>
                                <Button variant="default" className="bg-primary hover:bg-primary/90 text-white font-black px-8 h-12 rounded-xl w-full sm:w-auto" onClick={() => toast.success("Processando XML...")}>
                                    SELECIONAR ARQUIVO
                                </Button>
                            </div>
                        </CardContent>
